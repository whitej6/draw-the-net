...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_DRAW_THE_NET_xxx
namespace = Collection("draw_the_net")
namespace.configure(
    {
        "draw_the_net": {
            "nautobot_ver": "1.6.1",
            "project_name": "draw_the_net",
            "python_ver": "3.8",
            "local": False,
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.draw_the_net.compose_http_timeout,
        "NAUTOBOT_VER": context.draw_the_net.nautobot_ver,
        "PYTHON_VER": context.draw_the_net.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.draw_the_net.project_name}",
        f'--project-directory "{context.draw_the_net.compose_dir}"',
    ]

    for compose_file in context.draw_the_net.compose_files:
        compose_file_path = os.path.join(context.draw_the_net.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.draw_the_net.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.draw_the_net.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations draw_the_net"
...
def dbshell(context, query="", input="", output=""):
    ...
    if "docker-compose.mysql.yml" in context.draw_the_net.compose_files:
    ...
    elif "docker-compose.postgres.yml" in context.draw_the_net.compose_files:
        command += [
            "--",
            service,
            "psql",
...
def pylint(context):
    ...
    command = 'pylint --init-hook "import nautobot; nautobot.setup()" --rcfile pyproject.toml draw_the_net'
    run_command(context, command)
...
def unittest(context, keepdb=False, label="draw_the_net", failfast=False, buffer=True, pattern=""):
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'draw_the_net/*' --omit *migrations*"
...
def tests(context, failfast=False):
    ...
    if not is_truthy(context.draw_the_net.local):
